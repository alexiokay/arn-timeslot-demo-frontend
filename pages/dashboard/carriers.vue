...
div.settings(class="w-full h-full flex flex-col  px-4")
    ...
    div(class="bg-themeBackground2 w-full rounded-lg h-full p-3 border-themeBorder border-2 -mt-[0.3rem] ")
        div(class="w-full flex  ")
            div#shipments-menu-sorting(class="  px-2 flex items-center justify-center space-x-2 w-auto h-[2.5rem] rounded-lg bg-themeBackground2 text-themeText")
                p(class="text-sm ") Sort by:
                select#sorting(class="w-auto h-[2.5rem] focus:border-0 focus:outline-none text-themeText bg-themeBackground2")
                    option(value="newest") Newest
                    option(value="oldest") Oldest
            div#shipments-menu-sorting(class="  px-2 flex items-center justify-center space-x-2 w-auto h-[2.5rem] rounded-lg bg-themeBackground2 text-themeText")
                p(class="text-sm") Filter:
                select#filter(class="w-auto h-[2.5rem] focus:border-0 focus:outline-none text-themeText  bg-themeBackground2" v-model="choosedFilter")
                    option(value="") All
                    option(v-for="carrier in filters" :value="carrier.name") {{ carrier.name }}
            Searchbar(class="ml-3 h-[2.5rem] w-1/3 border-themeBorder2" :datepicker="false" placeholder="Search by name or email" v-model='searchValue')
            ...
            table(class=" table-auto w-full") <!--  fixed/auto -->
                thead(class="border-b-2 border-themeBorder2")
                    tr  
                        ...
                        th(class="px-4 py-2 border-r border-themeBorder w-[18%]") Carrier Name
                ...
                tbody(class="")
                    ...
                    tr(v-else v-for="(account, index) in filteredCarriers" :key="account" class="border-t " :id="detectEndOfList(index)")
                        ...
                        td(class="border-r border-themeBorder px-4 py-2") 
                          button.activate-account(v-if="account.is_activated && canUserActivate(account)" @click="_activateCarrier(account)" class="border-2 border-themeBorder rounded-md px-6 py-1 font-semibold hover:bg-blue-400 hover:text-white") Deactivate
                          button.activate-account(v-if=" !account.is_activated && canUserActivate(account)" @click="_activateCarrier(account)" class="border-2 border-themeBorder rounded-md px-6 py-1 font-semibold hover:bg-blue-400 hover:text-white") Activate
