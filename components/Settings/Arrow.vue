    ...
    template(v-slot:content)
        div(class="flex w-4/5 space-x-[2rem] items-start ")
            div(class="flex flex-col w-1/3")
                p(class="font-semibold text-themeText") Workable times
           
                p(class="text-themeText text-sm") Here you can set your default workable times. These times will be used for all your future bookings.
            div(class="flex flex-col w-auto  space-y-4  justify-center items-start ")
                div(class="flex flex-col w-auto  justify-center items-start ")
                    p Weekdays
                    hr(class="w-full border-themeBorder border-[1px]")
                ...
                div(class="flex  w-full items-center space-x-2")
                    ...
                    select(data-te-select-init class="h-[2rem]  w-[4.5rem,] bg-themeBackground p-1" v-model="settingsStore.workable_hours.from")
                ...
                div(class="flex  w-full items-center justify-start space-x-2")
                    ...
                    select(data-te-select-init class="h-[2rem]  w-[4.5rem,] bg-themeBackground p-1" v-model="settingsStore.workable_hours.to")
                        option(v-for="i in 24" :key="i") {{ i }}:00
            div(class="flex flex-col w-auto  space-y-4  justify-center items-start ")
                div(class="flex flex-col w-auto  justify-center items-start ")
                    p Saturdays
                ...
                div(class="flex  w-full items-center space-x-2")
                    label(data-te-select-label-ref class="w-[3rem]") from:
                    select(data-te-select-init class="h-[2rem]  w-[4.5rem,] bg-themeBackground p-1" v-model="settingsStore.saturday_workable_hours.from")
                        option(v-for="i in 24" :key="i") {{ i }}:00
                        
                div(class="flex  w-full items-center justify-start space-x-2")
                    label(data-te-select-label-ref class="w-[3rem]") to:
                    select(data-te-select-init class="h-[2rem]  w-[4.5rem,] bg-themeBackground p-1" v-model="settingsStore.saturday_workable_hours.to")
                        option(v-for="i in 24" :key="i") {{ i }}:00
            div(class="flex flex-col w-auto  space-y-4  justify-center items-start ")
                div(class="flex flex-col w-auto  justify-center items-start ")
                    p Sundays
                ...
                div(class="flex  w-full items-center space-x-2")
                    label(data-te-select-label-ref class="w-[3rem]") from:
                    select(data-te-select-init class="h-[2rem]  w-[4.5rem,] bg-themeBackground p-1" v-model="settingsStore.sunday_workable_hours.from")
                        option(v-for="i in 24" :key="i") {{ i }}:00
                        
                div(class="flex  w-full items-center justify-start space-x-2")
                    label(data-te-select-label-ref class="w-[3rem]") to:
                    select(data-te-select-init class="h-[2rem]  w-[4.5rem] bg-themeBackground p-1" v-model="settingsStore.sunday_workable_hours.to")
                        option(v-for="i in 24" :key="i") {{ i }}:00
        hr(class="w-full border-themeBorder border-[1px]")
        
        div(class="flex w-4/5 space-x-[4rem] items-center ")
            div(class="flex flex-col w-1/3")
                p(class="font-semibold text-themeText ") Max amount of trucks
           
                p(class="text-themeText text-sm") Here you can set your default max amount of trucks. This amount will be used for all your future bookings.
            div(class="flex flex-col w-auto space-x-8 space-y-4 ")
                input(class="h-[2rem] w-[3.6rem] bg-themeBackground  pl-2" type="number"  v-model="settingsStore.max_trucks_per_hour" min="1" max="10")
